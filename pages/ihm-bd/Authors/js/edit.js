...
$(document).ready(function() {
    ...
    $('#remove').click(function(e){
        ...
        $('#co'+(nbco+1)).remove();
        if(nbco!=1){
            $('#co'+(nbco-1)).after("<span id=\"co"+nbco+"\"></span>");
        }else{
            $('#debut').after("<span id=\"co"+nbco+"\"></span>")
            $('#remove').prop('disabled', true);
        }
    ...
    $("#next_3").click(function(e) {
        ...
        $('#keyword_article').text($('#Keywords').val());
        $('#editor_article').text($('#editor option:selected').text());
        for (var p=1;p<=3;p++){
            $("#re_fn"+p+"_article").text($("#re_fn"+p).val());
            $("#re_mn"+p+"_article").text($("#re_mn"+p).val());
            $("#re_ln"+p+"_article").text($("#re_ln"+p).val());
            $("#re_ln"+p+"_article").text($("#re_ln"+p).val());
            $("#re_em"+p+"_article").text($("#re_em"+p).val());
        }
            $("#co_auteur").html('');
    ...
    $("#next_4").click(function(e) {
        ...
        main_file=main_file.replace(/ /g, "_");
        ...
        data_sub=data_sub+"&etat_editor="+etat_editor;
        console.log(data_sub);
        $.ajax({
            type:"POST",
            url:"php/validate_edit.php",dataType: "json",
            data: data_sub,async:false,
            success:function(data)
            {
                if(data[0] == 1)
                {
                    window.location.href="php/track.php?id="+ida ;
                }
                else
                {
            ...
            if (res){
                ...
                request.upload.addEventListener('progress',function(event) {
                    if (event.lengthComputable) {
                        ...
                        $('#state_progresse').width(progresse+'%');
                        if(progresse==100) {
                            $('#add_images').show();
                            setTimeout(function()
                            {
