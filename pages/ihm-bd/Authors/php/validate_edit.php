...
$filename=$filedec[0].".pdf";
    ...
    //insertion article
            ...
            $inser = $conn->query("UPDATE article SET author='".$idauthor."', title='".$title."', type='".$type."', area='".$area."', abstract='".$abstract."', keywords='".$keywords."', date=now(),state='With Editor' WHERE id=".$idarticle);
                        ...
                        for($i=1;$i<=$nb_co;$i++)
                        {
                            ...
                            $inser = $conn->query("INSERT INTO co_author (fname, mname, lname, affiliation,adresse,email,article) VALUES ('".$fname."','".$mname."','".$lname."','".$affiliation."','".$address1."','".$email."',".$idarticle.")");}
                                if($inser){
                                        ...
                                        $inserfile=$conn->query("INSERT INTO file (name, size,article,type) VALUES ('ajam_D_".$idarticle.".pdf','',".$idarticle.",'AJAM-D')");
                                        rename("../files/".$idauthor."/".$idarticle."/source/temp/".$filename."","../files/".$idauthor."/".$idarticle."/ajam_D_".$idarticle.".pdf");
                                    //ajoute premiere page
                                    for($i=1;$i<=$nb_co;$i++){
...
<td>".$keywords."</td>
                                    ...
                                    $ajam_d=mysqli_insert_id($conn);
                                        ...
                                        //$inserfile=$conn->query("UPDATE state_author SET date=now(),state='Submited Article', file='".$ajam_d."' WHERE id=".$id_etat);
