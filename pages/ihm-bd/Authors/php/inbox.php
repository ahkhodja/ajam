...
$select = $conn->query("SELECT src,msg, DATE_FORMAT(date, '%d %M') as date FROM messages WHERE dis=".$_SESSION['id']);
if ($select) {
    $contenu = $contenu . "
...
{$contenu = $contenu . "<td colspan=4>There are no messages in this list.</td>";}
