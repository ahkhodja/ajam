...
if(isset($_GET['c1'])&&isset($_GET['c2'])&&isset($_GET['rsa'])){
    ...
    if($select) {
        ...
        if($row["date_accepte"]==null){
            ...
            if($update){
                $count=$conn->query("SELECT id from state_review WHERE article=".$row['article']." AND state !='Null'");
                if($count){
                    ...
                    if(mysqli_num_rows($count)==1){
                        ...
                        $insert_state_editor=$conn->query("INSERT INTO state_editor(article,file,etat,date,editor) VALUES (".$row['article'].",".$file.",'Draft in review',now(),".$editor.")");
                        $update_state_editor=$conn->query("UPDATE state_editor SET etat='-Draft Submited-' WHERE id=".$et_ed);
...
<script src="js/rev_validate.js" type="application/javascript"></script>
</body>
</html>
