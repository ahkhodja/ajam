...
if(isset($Novelty)&&isset($Technical)&&isset($Relevance)&&isset($Quality)&&isset($comment)&&isset($recommendation)&&isset($Confidential)){
    ...
    if($rev){
        ...
        if(!$res) {
            ...
            $select_review_state=$conn->query("SELECT state FROM state_review WHERE article=".$article."  AND type='Review 2' AND date_result IS null");
            if($select_review_state){
                ...
                if($termine){
                    $update_editor=$conn->query("UPDATE state_editor SET etat='-R1 in review-' WHERE article=".$article." and etat='R1 in review'");

                    $insert_state_editor=$conn->query("INSERT INTO state_editor(article,file,etat,date,editor) VALUES (".$article.",".$file.",'R1 in Decision',now(),".$editor.")");
