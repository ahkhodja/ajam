...
if(isset($_GET['c1'])&&isset($_GET['c2'])&&isset($_GET['rsa'])){
    ...
    $select=$conn->query("SELECT article,file,date_accepte,date_result,reviewer,date,state FROM state_review WHERE cle='".$cle."'AND cle2='".$cle2."'AND id=".$id);
    ...
    if($select) {
        ...
        if($row["date_accepte"]==null){
            ...
            if($update){
                $body="Dear Dr. ".$row_rev['fname']." ".$row_rev['lname'].",</br>

Thank you for agreeing to review manuscript".$row_article['etiquette'].$row['article']." for Algerian Journal od Advanced Materials. 

If possible, I would appreciate receiving your review by 30 May 2015.  </br> 

You may submit your comments online at http://ajam.revsys.dz/. </br>

There you will find spaces for confidential comments to the editor, comments for the author and a report form to be completed. </br>

With kind regards </br>

Pr ".$row_editor['fname']." ".$row_editor['lname']."
Associate Editor 
...
";
                ...
                $count=$conn->query("SELECT id from state_review WHERE article=".$row['article']." AND state !='NULL' AND type='Review 3'");
                if($count){
                    ...
                    if(mysqli_num_rows($count)==1){
                        ...
                        $insert_state_editor=$conn->query("INSERT INTO state_editor(article,file,etat,date,editor) VALUES (".$row['article'].",".$file.",'R2 in review',now(),".$editor.")");
                        $update_state_editor=$conn->query("UPDATE state_editor SET etat='-R2 Submited-' WHERE id=".$et_ed);
    ...
    <div class="col-xs-10">
        <div class="row">
            ...
            <form class="form-horizontal col-lg-12" id="form" method="post">
                ...
                <div class="radio">
                    <label><input type="radio" name="Novelty" value="A pioneering piece of work.">A pioneering piece of work.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Novelty" value="Novel results and original work">Novel results and original work.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Novelty" value="Some novel results on a subject well investigated.">Some novel results on a subject well investigated.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Novelty" value="Minor variations on a well studied subject.">Minor variations on a well studied subject.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Novelty" value="It has been said many times before.">It has been said many times before.</label>
                </div>
                </br>
                ...
                <div class="radio">
                    <label><input type="radio" name="Technical" value="Excellent technical quality.">Excellent technical quality.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Technical" value="Technically solid.">Technically solid.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Technical" value="Minor flaws, but conclusions still believable.">Minor flaws, but conclusions still believable.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Technical" value="Significant flaws that may call some results into question.">Significant flaws that may call some results into question.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Technical" value="Questionable work with severe flaws.">Questionable work with severe flaws.</label>
                </div>
                </br>
                ...
                <div class="radio">
                    <label><input type="radio" name="Quality" value="Readable; could benefit from revision.">Readable; could benefit from revision.</label>
                </div>
                <div class="radio">
                    <label><input type="radio" name="Quality" value="Substantial revision work is needed.">Substantial revision work is needed.</label>
                </div>
                <div class="radio">
                ...
                <textarea name="comment"  class="form-control" id="Comment" placeholder ="Comments and Recommended Changes  ... " rows="8"></textarea>
                </br>
                ...
                <textarea name="Confidential"  class="form-control" id="Confidential" placeholder ="Confidential Comments ... " rows="8"></textarea>
                </br>
...
<script src="js/rev_validate3.js" type="application/javascript"></script>
</body>
</html>
